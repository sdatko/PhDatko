\documentclass[a4paper,12pt,oneside,openright]{report}
% \documentclass[a4paper,12pt,twoside,openright]{report}

\usepackage{cmap}
\usepackage[english]{babel}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{lmodern}
\usepackage{microtype}
\usepackage{setspace}
\usepackage[dvipsnames]{xcolor}

\usepackage[a4paper,
            bindingoffset=0.5cm,
            top=2.5cm,
            bottom=2.5cm,
            left=2.5cm,
            right=2.5cm,
            marginparwidth=2.0cm]{geometry}

\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{amsthm}
\usepackage{array}
\usepackage{caption}
\usepackage{changepage}
\usepackage{enumitem}
\usepackage{esvect}
\usepackage{float}
\usepackage{graphicx}
\usepackage[bookmarks,pdfusetitle,unicode]{hyperref}
% \usepackage[none]{hyphenat}  % turn off hyphenation (words splitting)
\usepackage{hyphenat}
\usepackage{indentfirst}
\usepackage{lipsum}
\usepackage{longtable}
\usepackage[newfloat]{minted}
\usepackage[all,defaultlines=3]{nowidow}  % prevent single lines between pages
\usepackage{physics}
% \usepackage{showlabels}  % TODO comment/remove at the end
% \usepackage{siunitx}
\usepackage{subcaption}
\usepackage{tikz}
\usepackage[colorinlistoftodos,textsize=tiny]{todonotes}
\usepackage{ulem}

\usepackage{booktabs}
\usepackage{multirow}
\usepackage{mparhack}

% reduce the hyphenation to only where it is *really* necessary
% but do not remove it completely – to avoid the Overfull \hbox
\hyphenpenalty=1000

% dots in the table of contents
\usepackage{tocloft}
\renewcommand{\cftchapdotsep}{\cftdotsep}

% headers customization
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhf{}  % clear all current header and footer fields

\renewcommand{\headrulewidth}{1.0pt}
\renewcommand{\chaptermark}[1]{\markboth{\MakeUppercase{\thechapter.~#1}}{}}
\fancyhead[LE,RO]{\bfseries\thepage}  % page number: left on even, right on odd
\fancyhead[RE]{\bfseries\nouppercase{\leftmark}\chaptermark}  % right on even
\fancyhead[LO]{\bfseries\nouppercase{\rightmark}\sectionmark}  % left on odd

\fancypagestyle{plain}{% plain = e.g. clear pages and where chapters begin
    \fancyhf{}
    \fancyhead[RO]{\bfseries\thepage}
    % \renewcommand{\headrulewidth}{0pt}
}

% footnotes customization
\setlength{\footnotesep}{10pt}

\renewcommand{\footnoterule}{
    \kern 12pt
    \hrule width \textwidth height 0.4pt
    \kern 5pt
}

\interfootnotelinepenalty=10000  % do not split between pages

% do not reset footnote counter between chapters
\usepackage{chngcntr}
\counterwithout{footnote}{chapter}

% prefer leaving empty space on the bottom of the page
% rather than expading the spaces between elements
\raggedbottom

% allow marking pages intentionally left blank
\makeatletter
    \def\markIntentionallyBlankPages{
        \def\cleardoublepage{\clearpage%
            \if@twoside
                \ifodd\c@page\else
                    \vspace*{\fill}
                    {
                        \centering
                        (This page is intentionally left blank)
                        \par
                    }
                    \vspace{\fill}
                    \newpage
                    \if@twocolumn\hbox{}\newpage\fi
                \fi
            \fi
        }
    }
\makeatother

% captions customization
\captionsetup{justification=centering}
\captionsetup{font=small,format=plain,labelfont=bf}
\captionsetup{belowskip=2.0em}
\captionsetup[subfigure]{belowskip=0em}

% dotted borders around links in PDF viewers
\hypersetup{pdfborderstyle=/S/D/D[3 2]/W 0.25,pdfborder=0 0 1}

% no borders around links in PDF viewers
% \hypersetup{colorlinks,allcolors=black}

% restore the \qty command from siunitx package
% \AtBeginDocument{\RenewCommandCopy\qty\SI}

% amsthm confiugration
\newtheorem{theorem}{Theorem}
\newtheorem{corollary}{Corollary}
\newtheorem{lemma}{Lemma}

\theoremstyle{definition}
\newtheorem{definition}{Definition}[section]

% \theoremstyle{remark}
\newtheorem*{remark}{Remark}

% avoid moving figures to separate pages
% \renewcommand{\topfraction}{.8}
% \renewcommand{\floatpagefraction}{.8}

% miscellaneous and additional text formatting
\setstretch{1.25}
\captionsetup{font={stretch=1.2}}
\setlength{\parskip}{0.75em}
\setlength{\headheight}{15pt}
\addtolength{\topmargin}{-3pt}

\setminted{fontsize=\small,baselinestretch=1}

% custom helper commands
\newcommand{\best}[1]{\textcolor{Green}{\textbf{#1}}}
\newcommand{\worst}[1]{\textcolor{Red}{\textit{#1}}}


%
% Metadata
%
\title{Klasyfikacja z grupą otwartą w danych wysokowymiarowych}
\author{Szymon Tomasz Datko}
\date{2024}

% \def\TheUniversity{Politechnika Wrocławska}
\def\TheUniversity{Wrocław University of Science and Technology}
% \def\TheFaculty{Wydział Informatyki i Telekomunikacji}
\def\TheFaculty{Faculty of Information and Communication Technology}
% \def\TheHeader{Rozprawka Doktorska}
\def\TheHeader{Doctoral Thesis}

\def\TheTitleEN{Open-set classification \\
                of high-dimensional data}
\def\TheTitlePL{Klasyfikacja z grupą otwartą \\
                w danych wysokowymiarowych}

\def\TheAuthorLabel{Author}
% \def\TheAuthorTitle{mgr inż.}
\def\TheAuthorTitle{MSc Eng}
\def\TheAuthorName{Szymon Tomasz Datko}

\def\TheSupervisorLabel{Supervisor}
% \def\TheSupervisorTitle{dr hab. inż.}
\def\TheSupervisorTitle{PhD DSc Eng}
\def\TheSupervisorName{Henryk Maciejewski}

\def\TheDate{Wrocław 2024}

\def\TheSentenceText{We don't make mistakes; \\
                     we just have happy accidents.}
\def\TheSentenceAuthor{Bob Ross}


%
% Content
%
\begin{document}
    {%
        \pagestyle{empty}

        \include{chapters/00a-title}
        \cleardoublepage

        \include{chapters/00b-sentence}
        \cleardoublepage
    }


    \pagenumbering{roman}
    \setcounter{page}{1}
    \markIntentionallyBlankPages

    \include{chapters/00c-acknowledgments}
    \cleardoublepage

    \include{chapters/00d-abstract}
    \cleardoublepage

    \include{chapters/00e-streszczenie}
    \cleardoublepage


    \pagenumbering{arabic}
    \setcounter{page}{1}

    \include{chapters/00f-contents}
    \cleardoublepage

    \include{chapters/01-introduction}
    \cleardoublepage

    \include{chapters/02-related-work}
    \cleardoublepage

    \include{chapters/03-simulations}
    \cleardoublepage

    \include{chapters/04-real-data}
    \cleardoublepage

    \include{chapters/05-summary}
    \cleardoublepage


    \appendix

    \include{chapters/81-glossary}
    \cleardoublepage

    \include{chapters/82-source-code}
    \cleardoublepage

    \include{chapters/83-achievements}
    \cleardoublepage

    \include{chapters/97-figures}
    \cleardoublepage

    \include{chapters/98-listings}
    \cleardoublepage

    \include{chapters/99-bibliography}
    \cleardoublepage
\end{document}
